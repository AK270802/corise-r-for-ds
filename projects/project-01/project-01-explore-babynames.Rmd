...
plot_trends_in_first_letter <- function(my_letter) {
  tbl_names_by_year_first_letter <- tbl_names |> 
    # Filter for names starting with `my_letter`
    filter(str_sub(name, 1, 1) == my_letter) |> 
    # Group by year and sex
    group_by(year, sex) |> 
    # Summarize total births in the year by sex
    summarize(total_births = sum(nb_births)) |> 
    # Add NEW column `perc_births` for the percentage of total births by sex
    mutate(perc_births = total_births / sum(total_births)) |> 
    # Convert year to a factor for plotting
    mutate(year = factor(year)) |> 
    # Sort by year
    arrange(year)
  
  ggplot(tbl_names_by_year_first_letter, aes(x = year, y = perc_births, fill = sex)) +
    geom_bar(stat = "identity", position = "stack") +
    labs(
      title = glue::glue("Trends in births for names starting with {my_letter}"),
      x = "Year",
      y = "Percentage of total births",
      fill = "Sex"
    ) +
...
tbl_names_by_first_and_last_letter |> 
  ...
  labs(
    title = "Distribution of Births by First and Last Letters, 2021",
    subtitle = "Percentage of Births by First and Last Letter Combination",
    x = "Last Letter",
    y = "First Letter",
...
tbl_names_vowel_consonant |> 
  ...
  # Add labels (title, subtitle, caption, x, y)
  labs(
    title = "Trends in the usage of vowels and consonants in names over time",
    subtitle = "Percentage of births by the combination of first and last letter types, by sex",
    x = "Year",
